...
- 職業原本在 `/review` 寫死為 `cookery`，已由 PR #35 改成自動判斷。
...
**2026-09-22 早上，`fix/d7-interview-figma`（PR #37，待合入）**：`/interview` 網頁錄音畫面更貼近 Figma frame 10:83：面試中不顯示標頭、版面滿版，標籤只寫 Current Question，拿掉圓球上的 READY 字樣和卡片底部提示；兩顆圓鈕改成 Figma 的 ▶／■（▶ 開始錄音、■ 停止），題目不再自動朗讀，點圓球才唸（避免嚇到求職者）。電話面試與開始畫面未動。verify GREEN。
