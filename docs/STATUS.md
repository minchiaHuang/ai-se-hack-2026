...
- **題目選定：方向 6（WISE 就業成效申報）**，2026-09-21 下午決定。第二輪研究成果寫在
  ...
  4. ⛔ **2026-09-21 傍晚推翻自己的結論**：「沒有人做資助方格式對映」是錯的。
     Sopact／Knack／LiveImpact 都自述做了（廠商宣稱，未獨立驗證）。
     還站得住的只剩「**他們做的不是澳洲的格式**」＋「不服務 4–26 人的組織」＋
     「倫理紅線寫進程式」這三條。
  5. ⭐ **最強付費論證改為 SEDI**：2026-05-08 已開放的 2026–27 輪，每家最高 12 萬澳幣，
     用途明文含 evaluation and impact measurement，且**獲補助者須建立 Impact Measurement
     Framework**。付費意願寫在補助條款裡，不需要論證。
  - 產品地圖（五層）、申報格式欄位清單、2026-09 可申請補助清單：
    框架檔「市面產品地圖」「申報格式清單」「最強的付費論證」三節，出處在交接文件「第三輪研究」。
  - 一個立刻可用的產品約束：DSS 的 AWP／AWPR 範本**每個文字欄位建議 300 字以內**。
  - **評審三題（Problem Validation 10%／Fit 20%／Usability 15%＝45%）的完整答法**
    已寫在框架檔「評審三題的答法」一節，含要照念的英文句子與**我們自己先講的六個限制**。
    通則：主張→證據→自己先講弱點；PBO3 的「BAU」反證一定要自己講，不要等評審問。
  6. ⛔ **2026-09-21 深夜：「用 AI 幫填資料」不能當定位。** 那一格是滿的——
     **Drafter**（Our Community／SmartyGrants 姊妹單位，澳洲，2025-10 上線）、
     Grantyd（US$20/月，一鍵填回原表）、Grantable、Loopio、Instafill。
     **唯一站得住的區分是「文字題 vs 數字題」**：他們從內容庫重複利用散文，
     沒有一家宣稱從營運紀錄算出數字並附算式與來源。改口為「We compute the numbers」。
